 ...
 */
export function searchKnowledge(query: string, currentPath: string = ''): { response: string; suggestions: string[] } {
  const normQuery = normalizeText(query)
  if (!normQuery) {
    return {
      response: '¡Hola! Soy tu asistente de GFarma. ¿En qué módulo o funcionalidad tienes dudas hoy? Puedo ayudarte con las facturas, el escáner OCR, los gráficos de análisis, la fiscalidad o los trabajadores.',
      suggestions: ['¿Cómo escaneo una factura?', '¿Cómo funciona la fiscalidad?', '¿Cómo exportar a PDF?']
    }
  }

  // Si es un saludo sencillo
  const greetings = ['hola', 'buenos dias', 'buenas', 'buenas tardes', 'saludos', 'hi', 'hello']
  if (greetings.includes(normQuery)) {
    return {
      response: '¡Hola! Encantado de saludarte. ¿Cómo te puedo ayudar hoy con el funcionamiento de la aplicación?',
      suggestions: ['¿Cómo se suben facturas?', '¿Cómo se configuran presupuestos?']
    }
  }

  // Si agradecen
  const thanks = ['gracias', 'muchas gracias', 'perfecto', 'ok', 'de acuerdo', 'entendido']
  if (thanks.includes(normQuery)) {
    return {
      response: '¡De nada! Si tienes cualquier otra duda sobre las herramientas de GFarma, pregúntame sin compromiso.',
  ...
  return {
    response: 'Vaya, no he encontrado una respuesta exacta a tu pregunta en la base de conocimientos de GFarma. \n\nPrueba a preguntarme usando palabras clave más sencillas como **"escanear factura"**, **"exportar PDF"**, **"añadir impuesto"** o **"nóminas"**, o utiliza una de las sugerencias de abajo.',
    suggestions: ['¿Cómo se añade un impuesto?', '¿Cómo marco una factura pagada?', '¿Cómo funciona el escaneo OCR?']
  }
}
